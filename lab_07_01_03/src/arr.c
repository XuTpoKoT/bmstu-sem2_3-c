...
{
    int *pcur;  
    for (pcur = (int*)pb; pcur != (int*)pe && fscanf(f, "%d", pcur) == 1; ++pcur);
...
int last_neg_index(const int *pb, const int *pe)
{
    int *pcur;
...
int key(const int *pb_src, const int *pe_src, int **pb_dst, int **pe_dst)
{
    ...
    size_t last_ind = last_neg_index(pb_src, pe_src);
    if (last_ind == 0)
        return EXIT_FAILURE;

    int *pb = NULL, *pe = NULL; 
    pb = malloc(last_ind * sizeof(int));
    if (pb == NULL)
        return EXIT_FAILURE;
    pe = pb + last_ind;
